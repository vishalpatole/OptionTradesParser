...
namespace OptionTradesParser
{
    ...
    public static class ExpirationResolver
    {
        ...
        public static string? Resolve(string content)
        {
            ...
            // A fully dated contract line outranks a weekday mention, which in turn outranks a "0DTE" tag,
            // and then a bare month/day. This keeps generic weekday phrases like "Friday" or "Thursday"
            // resolving to the next matching expiry instead of falling back to the current day.
            ...
            string? weekdayResult = ResolveWeekday(content, today);
        ...
        private static string? ResolveWeekday(string content, DateTime today)
        {
            Match match = WeekdayRegex.Match(content);
            if (!match.Success) return null;

            string weekday = match.Groups["day"].Value;
            DayOfWeek target = weekday.ToLowerInvariant() switch
            {
                ...
                _ => throw new ArgumentOutOfRangeException(nameof(weekday), weekday, "Unsupported weekday token")
            };

            int daysAhead = ((int)target - (int)today.DayOfWeek + 7) % 7;
            if (daysAhead == 0) daysAhead = 7;

            return today.AddDays(daysAhead).ToString("yyyyMMdd");
        }
